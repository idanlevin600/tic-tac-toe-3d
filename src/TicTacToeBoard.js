...
const TicTacToeBoard = ({ face, board, position, rotation, onCellClick, winningCells, bombMode, bombCells, highlightedCells }) => {
  ...
  const renderCell = (cell, idx) => {
    ...
    const adjacentCells = hoveredFace !== null && hoveredCell !== null ? getAdjacentCells(hoveredFace, hoveredCell) : [];
    const isAdjacentHovered = adjacentCells.some(([adjFace, adjCell]) => adjFace === face && adjCell === idx);

    return (
      <group key={idx} position={getCellPosition(idx)}>
        <mesh
          userData={{ face, cell: idx }}
          onPointerOver={(e) => { setHoveredCell(idx); setHoveredFace(face); e.stopPropagation(); }}
          onPointerOut={(e) => { setHoveredCell(null); setHoveredFace(null); e.stopPropagation(); }}
